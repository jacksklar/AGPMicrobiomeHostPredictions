...
#cohort_matching_features = ["sex", "age_years", "bmi", "longitude", "latitude", "alcohol_frequency", "milk_cheese_frequency", "bowel_movement_quality", "meat_eggs_frequency", "vegetable_frequency"]  #"meat_eggs_frequency",
...
def buildDataSubset(metadata, target_var, pos_target, neg_target, exclude_vars): #age_related, bmi_related, location_related):
    ...
    if exclude_vars is not None:    
        cm = euclidean_distances(metadata_matching.loc[pos_class, :].drop(exclude_vars, axis = 1),  metadata_matching.loc[neg_class, :].drop(exclude_vars, axis = 1)) 
    else:
...
#save_path = "/Users/sklarjg/Desktop/MICROBIOME/AmericanGutProj/Feature_Cohorts/binary_cohorts_no_matching/"
#save_path = "/Users/sklarjg/Desktop/MICROBIOME/AmericanGutProj/Feature_Cohorts/binary_cohorts_standard/"
...
metadata_df_healthy = metadata_df_healthy.loc[:, cohort_feats]
metadata_df_healthy.to_csv("/Users/sklarjg/Desktop/MICROBIOME/AmericanGutProj/Feature_cohorts/Metadata_distances/selection_pop_metadata.csv")

metadata_df_healthy = metadata_df[(metadata_df["antibiotic_history"].isin(["Year", "I have not taken antibiotics in the past year."])) &
                                  (metadata_df["ibd"] == 0) &
                                  (metadata_df["diabetes"] == 0) &
                                  (metadata_df["bmi"] >= 12.5) & 
                                  (metadata_df["bmi"] <= 40.0)]
metadata_df_healthy = metadata_df_healthy.loc[:, cohort_feats]
metadata_df_healthy.to_csv("/Users/sklarjg/Desktop/MICROBIOME/AmericanGutProj/Feature_cohorts/Metadata_distances/selection_pop_metadata_all_ages.csv")

metadata_df_healthy = metadata_df[(metadata_df["bmi"] >= 12.5) & (metadata_df["bmi"] <= 50.0)]
metadata_matching_full = metadata_matching.loc[metadata_df_healthy.index, :]
cm = euclidean_distances(metadata_matching_full) 
cm = pd.DataFrame(cm, index = metadata_matching_full.index, columns = metadata_matching_full.index)
cm.to_csv("/Users/sklarjg/Desktop/MICROBIOME/AmericanGutProj/Feature_Cohorts/Metadata_distances/full_pop_distances.csv")
...
young_cohort = buildDataSubset(metadata_df_healthy_young, "age_group", 1, 0, ["age_years", "bmi"])
...
infant_cohort = buildDataSubset(metadata_df_healthy_toddler, "age_group", 1, 0, ["age_years", "bmi"])
...
infant_cohort = buildDataSubset(metadata_df_healthy_infant, "age_group", 1, 0, ["age_years", "bmi"])
...
old_cohort = buildDataSubset(metadata_df_healthy_old, "age_group", 1, 0,  ["age_years"])
...
obese_cohort = buildDataSubset(metadata_df_healthy, "bmi_cat", "Obese", "Normal", ["bmi"])
obese_cohort.to_csv(save_path + "Obese_cohort.csv")
#metadata_df_healthy.to_csv(save_path + "Obese_cohort.csv", columns=[], header=False)

print ("Constructing Overweight cohort with matched normal samples")
overweight_cohort = buildDataSubset(metadata_df_healthy, "bmi_cat", "Overweight", "Normal", ["bmi"])
overweight_cohort.to_csv(save_path + "Overweight_cohort.csv")
#metadata_df_healthy.to_csv(save_path + "Overweight_cohort.csv", columns=[], header=False)

print ("Constructing Underweight cohort with matched normal samples")
underweight_cohort = buildDataSubset(metadata_df_healthy, "bmi_cat", "Underweight", "Normal", ["bmi"])
underweight_cohort.to_csv(save_path + "Underweight_cohort.csv")
#metadata_df_healthy.to_csv(save_path + "Underweight_cohort.csv", columns=[], header=False)
...
#metadata_df_healthy.to_csv(save_path + "IBD_cohort.csv", columns=[], header=False)
...
#metadata_df_healthy.to_csv(save_path + "antiB_Year_cohort.csv", columns=[], header=False)
...
#metadata_df_healthy.to_csv(save_path + "antiB_6Month_cohort.csv", columns=[], header=False)
...
#metadata_df_healthy.to_csv(save_path + "antiB_Month_cohort.csv", columns=[], header=False)
...
#metadata_df_healthy.to_csv(save_path + "antiB_Week_cohort.csv", columns=[], header=False)
...
#metadata_df_healthy.to_csv(save_path + "diabetes_typeII_cohort.csv", columns=[], header=False)
...
#metadata_df_healthy.to_csv(save_path + "gender_healthy_cohort.csv", columns=[], header=False)
...
#metadata_df_healthy.to_csv(save_path + "pill_cohort.csv", columns=[], header=False)
...
#metadata_df_healthy.to_csv(save_path + "iud_cohort.csv", columns=[], header=False)
...
#metadata_df_healthy.to_csv(save_path + "asd_cohort.csv", columns=[], header=False)
...
bmq_solid_cohort = buildDataSubset(metadata_df_healthy, "bowel_movement_quality", 2, 1, ["bowel_movement_quality"])
bmq_solid_cohort.to_csv(save_path + "bmq_solid_cohort.csv")

print ("Constructing BMQ loose cohort with matched healthy samples")
bmq_loose_cohort = buildDataSubset(metadata_df_healthy, "bowel_movement_quality", 0, 1, ["bowel_movement_quality"])
...
save_path = "/Users/sklarjg/Desktop/MICROBIOME/AmericanGutProj/Feature_Cohorts/Test_cohorts/"
...
drinker_type_cohort = buildDataSubset(metadata_df_healthy, "average_drinks_per_week", "light_drinker", "non_drinker", ["alcohol_frequency"])
drinker_type_cohort.to_csv(save_path + "light_drinker_type_cohort.csv")

drinker_type_cohort = buildDataSubset(metadata_df_healthy, "average_drinks_per_week", "mild_drinker", "non_drinker", ["alcohol_frequency"])
drinker_type_cohort.to_csv(save_path + "mild_drinker_type_cohort.csv")

drinker_type_cohort = buildDataSubset(metadata_df_healthy, "average_drinks_per_week", "heavy_drinker", "non_drinker", ["alcohol_frequency"])
drinker_type_cohort.to_csv(save_path + "heavy_drinker_type_cohort.csv")

#%%

save_path = "/Users/sklarjg/Desktop/MICROBIOME/AmericanGutProj/Feature_Cohorts/Diabetes_test_cohorts/"
save_path_2 = "/Users/sklarjg/Desktop/MICROBIOME/AmericanGutProj/Feature_Cohorts/Metadata_distances/Diabetes_test_distances/"
...
diabetes_cohort = buildDataSubset(metadata_df_healthy, "diabetes_type", "Type II diabetes", 0, None)
diabetes_cohort.to_csv(save_path + "diabetes_cohort_matched.csv")

diabetes_cohort = buildDataSubset(metadata_df_healthy, "diabetes_type", "Type II diabetes", 0, ["sex"])
diabetes_cohort.to_csv(save_path + "diabetes_cohort_sex.csv")

diabetes_cohort = buildDataSubset(metadata_df_healthy, "diabetes_type", "Type II diabetes", 0, ["age_years"])
diabetes_cohort.to_csv(save_path + "diabetes_cohort_age.csv")

diabetes_cohort = buildDataSubset(metadata_df_healthy, "diabetes_type", "Type II diabetes", 0, ["bmi"])
diabetes_cohort.to_csv(save_path + "diabetes_cohort_bmi.csv")

diabetes_cohort = buildDataSubset(metadata_df_healthy, "diabetes_type", "Type II diabetes", 0, ["alcohol_frequency"])
diabetes_cohort.to_csv(save_path + "diabetes_cohort_alc.csv") 

diabetes_cohort = buildDataSubset(metadata_df_healthy, "diabetes_type", "Type II diabetes", 0, ["milk_cheese_frequency"])
diabetes_cohort.to_csv(save_path + "diabetes_cohort_dairy.csv")

diabetes_cohort = buildDataSubset(metadata_df_healthy, "diabetes_type", "Type II diabetes", 0, ["meat_eggs_frequency"])
diabetes_cohort.to_csv(save_path + "diabetes_cohort_meat.csv")

diabetes_cohort = buildDataSubset(metadata_df_healthy, "diabetes_type", "Type II diabetes", 0, ["bowel_movement_quality"])
diabetes_cohort.to_csv(save_path + "diabetes_cohort_bowel_mov_q.csv")

diabetes_cohort = buildDataSubset(metadata_df_healthy, "diabetes_type", "Type II diabetes", 0, ["vegetable_frequency"])
diabetes_cohort.to_csv(save_path + "diabetes_cohort_veg.csv")

diabetes_cohort = buildDataSubset(metadata_df_healthy, "diabetes_type", "Type II diabetes", 0, ["sex", "age_years", "bmi", "alcohol_frequency", "milk_cheese_frequency", "meat_eggs_frequency", "bowel_movement_quality", "vegetable_frequency"])
diabetes_cohort.to_csv(save_path + "diabetes_cohort_unmatched.csv")

diabetics = metadata_df_healthy[metadata_df_healthy["diabetes_type"] == "Type II diabetes"].index
controls = metadata_df_healthy[metadata_df_healthy["diabetes_type"] ==  0].index

metadata_matching_sub = metadata_matching.loc[metadata_df_healthy.index, :]
cm = euclidean_distances(metadata_matching_sub.loc[diabetics, :], metadata_matching_sub.loc[controls, :]) 
cm = pd.DataFrame(cm, index = diabetics, columns = controls)
cm.to_csv(save_path_2 + "diabetes_distance_matched.csv")

metadata_matching_sub = metadata_matching.loc[metadata_df_healthy.index, :].drop(["sex"], axis = 1)
cm = euclidean_distances(metadata_matching_sub.loc[diabetics, :], metadata_matching_sub.loc[controls, :]) 
cm = pd.DataFrame(cm, index = diabetics, columns = controls)
cm.to_csv(save_path_2 + "diabetes_distance_sex.csv")

metadata_matching_sub = metadata_matching.loc[metadata_df_healthy.index, :].drop(["age_years"], axis = 1)
cm = euclidean_distances(metadata_matching_sub.loc[diabetics, :], metadata_matching_sub.loc[controls, :]) 
cm = pd.DataFrame(cm, index = diabetics, columns = controls)
cm.to_csv(save_path_2 + "diabetes_distance_age.csv")

metadata_matching_sub = metadata_matching.loc[metadata_df_healthy.index, :].drop(["bmi"], axis = 1)
cm = euclidean_distances(metadata_matching_sub.loc[diabetics, :], metadata_matching_sub.loc[controls, :]) 
cm = pd.DataFrame(cm, index = diabetics, columns = controls)
cm.to_csv(save_path_2 + "diabetes_distance_bmi.csv")

metadata_matching_sub = metadata_matching.loc[metadata_df_healthy.index, :].drop(["alcohol_frequency"], axis = 1)
cm = euclidean_distances(metadata_matching_sub.loc[diabetics, :], metadata_matching_sub.loc[controls, :]) 
cm = pd.DataFrame(cm, index = diabetics, columns = controls)
cm.to_csv(save_path_2 + "diabetes_distance_alc.csv")

metadata_matching_sub = metadata_matching.loc[metadata_df_healthy.index,  :].drop(["milk_cheese_frequency"], axis = 1)
cm = euclidean_distances(metadata_matching_sub.loc[diabetics, :], metadata_matching_sub.loc[controls, :]) 
cm = pd.DataFrame(cm, index = diabetics, columns = controls)
cm.to_csv(save_path_2 + "diabetes_distance_dairy.csv")

metadata_matching_sub = metadata_matching.loc[metadata_df_healthy.index, :].drop(["meat_eggs_frequency"], axis = 1)
cm = euclidean_distances(metadata_matching_sub.loc[diabetics, :], metadata_matching_sub.loc[controls, :]) 
cm = pd.DataFrame(cm, index = diabetics, columns = controls)
cm.to_csv(save_path_2 + "diabetes_distance_meat.csv")

metadata_matching_sub = metadata_matching.loc[metadata_df_healthy.index, :].drop(["bowel_movement_quality"], axis = 1)
cm = euclidean_distances(metadata_matching_sub.loc[diabetics, :], metadata_matching_sub.loc[controls, :]) 
cm = pd.DataFrame(cm, index = diabetics, columns = controls)
cm.to_csv(save_path_2 + "diabetes_distance_bowel_mov_q.csv")

metadata_matching_sub = metadata_matching.loc[metadata_df_healthy.index, :].drop(["vegetable_frequency"], axis = 1)
cm = euclidean_distances(metadata_matching_sub.loc[diabetics, :], metadata_matching_sub.loc[controls, :]) 
cm = pd.DataFrame(cm, index = diabetics, columns = controls)
cm.to_csv(save_path_2 + "diabetes_distance_veg.csv")

metadata_matching_sub = metadata_matching.loc[metadata_df_healthy.index, ["longitude", "latitude"]]
cm = euclidean_distances(metadata_matching_sub.loc[diabetics, :], metadata_matching_sub.loc[controls, :]) 
cm = pd.DataFrame(cm, index = diabetics, columns = controls)
cm.to_csv(save_path_2 + "diabetes_distance_unmatched.csv")
...
metadata_df_healthy = metadata_df[(metadata_df["age_years"] >= 20.0) & 
                                  ...
                                  (metadata_df["bmi"] <= 40.0)]
#unmatched
metadata_matching_full = metadata_matching.loc[metadata_df_healthy.index, ["longitude", "latitude"]]
cm = euclidean_distances(metadata_matching_full) 
cm = pd.DataFrame(cm, index = metadata_matching_full.index, columns = metadata_matching_full.index)
cm.to_csv("/Users/sklarjg/Desktop/MICROBIOME/AmericanGutProj/Feature_Cohorts/Metadata_distances/selection_pop_dists_unmatched.csv")

#matched
metadata_matching_full = metadata_matching.loc[metadata_df_healthy.index, :]
cm = euclidean_distances(metadata_matching_full) 
cm = pd.DataFrame(cm, index = metadata_matching_full.index, columns = metadata_matching_full.index)
cm.to_csv("/Users/sklarjg/Desktop/MICROBIOME/AmericanGutProj/Feature_Cohorts/Metadata_distances/selection_pop_dists_matched.csv")

#sex
metadata_matching_full = metadata_matching.loc[metadata_df_healthy.index, :].drop(["sex"], axis = 1)
cm = euclidean_distances(metadata_matching_full) 
cm = pd.DataFrame(cm, index = metadata_matching_full.index, columns = metadata_matching_full.index)
cm.to_csv("/Users/sklarjg/Desktop/MICROBIOME/AmericanGutProj/Feature_Cohorts/Metadata_distances/selection_pop_dists_loo_sex.csv")

#age_years
metadata_matching_full = metadata_matching.loc[metadata_df_healthy.index, :].drop(["age_years"], axis = 1)
cm = euclidean_distances(metadata_matching_full) 
cm = pd.DataFrame(cm, index = metadata_matching_full.index, columns = metadata_matching_full.index)
cm.to_csv("/Users/sklarjg/Desktop/MICROBIOME/AmericanGutProj/Feature_Cohorts/Metadata_distances/selection_pop_dists_loo_age.csv")

#bmi
metadata_matching_full = metadata_matching.loc[metadata_df_healthy.index, :].drop(["bmi"], axis = 1)
cm = euclidean_distances(metadata_matching_full) 
cm = pd.DataFrame(cm, index = metadata_matching_full.index, columns = metadata_matching_full.index)
cm.to_csv("/Users/sklarjg/Desktop/MICROBIOME/AmericanGutProj/Feature_Cohorts/Metadata_distances/selection_pop_dists_loo_bmi.csv")

#alcohol_frequency
metadata_matching_full = metadata_matching.loc[metadata_df_healthy.index, :].drop(["alcohol_frequency"], axis = 1)
cm = euclidean_distances(metadata_matching_full) 
cm = pd.DataFrame(cm, index = metadata_matching_full.index, columns = metadata_matching_full.index)
cm.to_csv("/Users/sklarjg/Desktop/MICROBIOME/AmericanGutProj/Feature_Cohorts/Metadata_distances/selection_pop_dists_loo_alc.csv")

#milk_cheese_frequency
metadata_matching_full = metadata_matching.loc[metadata_df_healthy.index, :].drop(["milk_cheese_frequency"], axis = 1)
cm = euclidean_distances(metadata_matching_full) 
cm = pd.DataFrame(cm, index = metadata_matching_full.index, columns = metadata_matching_full.index)
cm.to_csv("/Users/sklarjg/Desktop/MICROBIOME/AmericanGutProj/Feature_Cohorts/Metadata_distances/selection_pop_dists_loo_dairy.csv")

#meat_eggs_frequency
metadata_matching_full = metadata_matching.loc[metadata_df_healthy.index, :].drop(["meat_eggs_frequency"], axis = 1)
cm = euclidean_distances(metadata_matching_full) 
cm = pd.DataFrame(cm, index = metadata_matching_full.index, columns = metadata_matching_full.index)
cm.to_csv("/Users/sklarjg/Desktop/MICROBIOME/AmericanGutProj/Feature_Cohorts/Metadata_distances/selection_pop_dists_loo_meat.csv")

metadata_matching_full = metadata_matching.loc[metadata_df_healthy.index, :].drop(["bowel_movement_quality"], axis = 1)
cm = euclidean_distances(metadata_matching_full) 
cm = pd.DataFrame(cm, index = metadata_matching_full.index, columns = metadata_matching_full.index)
cm.to_csv("/Users/sklarjg/Desktop/MICROBIOME/AmericanGutProj/Feature_Cohorts/Metadata_distances/selection_pop_dists_bowel_mov_q.csv")

#vegetable_frequency
metadata_matching_full = metadata_matching.loc[metadata_df_healthy.index, :].drop(["vegetable_frequency"], axis = 1)
cm = euclidean_distances(metadata_matching_full) 
cm = pd.DataFrame(cm, index = metadata_matching_full.index, columns = metadata_matching_full.index)
cm.to_csv("/Users/sklarjg/Desktop/MICROBIOME/AmericanGutProj/Feature_Cohorts/Metadata_distances/selection_pop_dists_loo_veg.csv")
